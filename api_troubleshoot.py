...
class FortiGateAPITroubleshooter:
    ...
    def test_token_formats(self):
        ...
        token_formats = [
            ...
            ("Authorization Header", lambda url, token: (url, {"Authorization": f"Bearer {token}"})),
            ("Authorization Basic", lambda url, token: (url, {"Authorization": f"Basic {base64.b64encode(f'{self.api_user}:{token}'.encode()).decode()}"})),
        ...
        for combo_name, token_value in combinations:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"   ❌ {combo_name}: HTTP {response.status_code} (Unauthorized)")
                else:
    ...
    def generate_diagnostic_report(self, token_results):
        ...
        if not working_formats:
            ...
            print("   - Ensure 'super_admin' profile or custom profile with read permissions")
