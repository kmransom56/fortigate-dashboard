...
def update_file(file_path):
    ...
    if "params" in content and "access_token" in content:
        ...
        if "test_with_query_param" in content and "test_with_auth_header" in content:
            logger.info(
                f"File {file_path} has both authentication methods for testing purposes. Not modifying."
            )
            return False
        ...
        while i < len(lines):
            ...
            if re.search(r'params\s*=\s*\{.*"access_token"', line) or re.search(
                r"params\s*=\s*\{.*'access_token'", line
            ):
                ...
                if token_match:
                    ...
                    updated_lines.append(
                        f"# UPDATED: Removed query parameter authentication"
                    )
                    ...
                    for j in range(i + 1, min(i + 5, len(lines))):
                        if "headers" in lines[j]:
                            ...
                            if re.search(r"headers\s*=\s*\{\s*\}", lines[j]):
                                # Empty headers
                                updated_lines.append(
                                    f'    headers = {{"Authorization": f"Bearer {token_var}"}}'
                                )
                                ...
                                if headers_line.endswith(","):
                                    updated_lines.append(
                                        f'{headers_line} "Authorization": f"Bearer {token_var}"}}'
                                    )
                                else:
                                    updated_lines.append(
                                        f'{headers_line}, "Authorization": f"Bearer {token_var}"}}'
                                ...
                                logger.warning(
                                    f"Complex headers format in {file_path}, manual review needed"
                                )
                        ...
                        updated_lines.append(
                            f'    headers = {{"Accept": "application/json", "Authorization": f"Bearer {token_var}"}}'
                        )
                        made_changes = True
                else:
