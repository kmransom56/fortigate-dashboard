...
def get_client_ip():
    ...
    try:
        ...
        if not in_allowed_range:
            logger.warning(
                "None of the local IPs are within the allowed range (192.168.0.0/24)"
            )
...
if __name__ == "__main__":
    ...
    if not any(ip.startswith("192.168.0.") for ip in all_ips + [local_ip]):
        logger.error(
            "ISSUE: IP Restriction - Your machine is not in the allowed network range (192.168.0.0/24)"
        )
        logger.error(
            "The FortiGate API is configured to only allow connections from 192.168.0.0/24"
        )
        logger.error(
            "You need to connect from a machine within that network or modify the API trusthost settings"
        )

    # Check authentication method
    if query_status != 200 and auth_status == 200:
        logger.error(
            "ISSUE: Authentication Method - The API requires the token in the Authorization header"
        )
        logger.error(
            "Modify your curl command to use: -H 'Authorization: Bearer hmNqQ0st7xrjnyQHt8dzpnkqm5hw5N'"
        )
    elif query_status == 200 and auth_status != 200:
        logger.error(
            "ISSUE: Authentication Method - The API requires the token as a query parameter"
        )
        logger.error(
            "Modify your application code to use the token as a query parameter"
        )
    elif query_status != 200 and auth_status != 200:
        logger.error(
            "ISSUE: Authentication Failed - Both methods failed, likely due to:"
        )
