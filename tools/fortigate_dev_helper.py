...
class FortiGateDevAPI:
        ...
        try:
            ...
            if method.upper() == "GET":
                ...
                response = self.session.post(
                    url, params=request_params, json=data, timeout=30
                )
            ...
            if response.status_code == 200:
                try:
                    ...
                    result["success"] = True
                    self.logger.info(
                        f"✅ {method} {endpoint} - {result['response_time']}ms"
                    )
    ...
    def monitor_endpoint(self, endpoint: str, interval: int = 30, duration: int = 300):
        ...
        while (time.time() - start_time) < duration:
            ...
            self.logger.info(
                f"📈 Monitored at {monitor_entry['timestamp']} - "
        ...
        monitor_file = Path(
            f'automation/logs/monitoring_{endpoint.replace("/", "_")}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
    ...
    def create_test_suite(self):
        ...
        # Save test results
        test_file = Path(
            f'automation/logs/test_suite_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
        ...
        self.logger.info(
            f"🧪 Test Suite Complete: {passed_tests}/{total_tests} tests passed"
        )
...
def main():
    ...
    parser.add_argument(
        "--test-connection", action="store_true", help="Test API connection"
    )
    parser.add_argument(
        "--test-all-endpoints", action="store_true", help="Test all endpoints"
    )
    parser.add_argument(
        "--export-responses", action="store_true", help="Export API responses"
    )
    parser.add_argument(
        "--monitor", type=str, help="Monitor endpoint (e.g., system/performance)"
    )
    parser.add_argument(
        "--test-suite", action="store_true", help="Run comprehensive test suite"
    )
    ...
    if args.test_connection:
        ...
        print(
            f"Test Suite Results: {sum(1 for test in results['tests'] if test['passed'])}/{len(results['tests'])} passed"
        )
