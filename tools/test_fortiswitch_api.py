    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, switch in enumerate(data):
                logger.info(f"Switch {i+1}: {switch.get('name')} ({switch.get('model')})")
                logger.info(f"  Status: {switch.get('status')}")
                logger.info(f"  IP: {switch.get('ip')}")
                logger.info(f"  Connected devices: {len(switch.get('connected_devices', []))}")
        else:
...
def test_switches_page():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to access FortiSwitch dashboard: {response.status_code}")
            logger.error(f"Response: {response.text[:200]}...")
    except Exception as e:
