...
import json
import logging
from app.services.fortiswitch_service import get_detected_devices, get_dhcp_info, process_fortiswitch_data
...
def main():
    ...
    if switches and len(switches) > 0:
        ...
        for device in switch['connected_devices']:
            logger.info(f"  - Device on port {device['port']}: {device['device_name']} (MAC: {device['device_mac']}, IP: {device['device_ip']})")
    else:
