...
class RedisSessionTestSuite:
    ...
    def test_redis_session_storage(self):
        ...
        try:
            ...
            if not session_data:
                return {
                    "success": False,
                    "message": "Failed to retrieve stored session",
                }

            # Validate session data
            if (
                session_data.session_key == test_session_key
                and session_data.fortigate_ip == test_ip
                and session_data.username == test_user
                and session_data.is_valid()
            ):
            ...
            return {
                "success": False,
                "message": f"Session storage test failed: {str(e)}",
            }
    ...
    def test_fortigate_session_manager(self):
        ...
        try:
            ...
            has_credentials = (
                health.get("fortigate_ip")
                and health.get("username")
                and health.get("has_password")
            )
            ...
            return {
                ...
                "details": {"health": health, "session_info": session_info},
            }

        except Exception as e:
            return {
                "success": False,
                "message": f"FortiGate session manager test failed: {str(e)}",
            }
    ...
    def test_session_expiration(self):
        ...
        try:
            ...
            if not store_result:
                return {
                    "success": False,
                    "message": "Failed to store test session for expiration",
                }
            ...
            if session_data.is_expired(buffer_minutes=0):
                ...
                return {
                    "success": False,
                    "message": "Session expiration detection failed",
                }

        except Exception as e:
            return {
                "success": False,
                "message": f"Session expiration test failed: {str(e)}",
            }
    ...
    def test_session_cleanup(self):
        ...
        try:
            ...
            for ip, user in test_sessions:
                self.redis_manager.store_session(
                    ip, user, f"session_{user}", expires_in_minutes=5
                )
            ...
            return {
                "success": False,
                "message": f"Session cleanup test failed: {str(e)}",
            }
    ...
    def test_concurrent_sessions(self):
        ...
        try:
                    ...
                    results.append(
                        {
                            ...
                            "retrieve_success": session_data is not None
                            and session_data.is_valid(),
                        }
                    )
                except Exception as e:
                    results.append({"thread_id": ip_suffix, "error": str(e)})
            ...
            successful_threads = sum(
                1
                for r in results
                if r.get("store_success") and r.get("retrieve_success")
            )
            ...
            return {
                "success": False,
                "message": f"Concurrent sessions test failed: {str(e)}",
            }
    ...
    def test_health_checks(self):
        ...
        try:
            ...
            session_manager_healthy = session_health.get(
                "fortigate_ip"
            ) and session_health.get("username")
