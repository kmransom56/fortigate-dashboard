...
def check_ip_in_network(ip, network, netmask):
    ...
    try:
        ...
        ip_int = sum(
            [int(octet) << (8 * (3 - i)) for i, octet in enumerate(ip.split("."))]
        )
        network_int = sum(
            [int(octet) << (8 * (3 - i)) for i, octet in enumerate(network.split("."))]
        )
        netmask_int = sum(
            [int(octet) << (8 * (3 - i)) for i, octet in enumerate(netmask.split("."))]
        )
...
def test_api():
    ...
    if not in_allowed_network:
        ...
        logger.error(
            "The Fortigate API is configured to only allow access from this network"
        )
        logger.error("You need to either:")
        logger.error("1. Connect from a device in the 192.168.0.0/24 network")
        logger.error(
            "2. Update the Fortigate API user trusthost configuration to include your IP"
        )
        return
    ...
    try:
        ...
        if CERT_PATH and os.path.exists(CERT_PATH):
            ...
            logger.warning(
                "No certificate path found or file doesn't exist. Disabling SSL verification."
            )
            response = requests.get(url, headers=headers, verify=False)
