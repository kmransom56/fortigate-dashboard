...
def check_fortigate_credentials():
    ...
    try:
        ...
        if (
            ...
            logger.error(
                f"   Password: {'Present' if health.get('has_password') else 'Missing'}"
            )
            return False
    ...
    try:
        ...
        if "error" in result:
            ...
            if error_type == "authentication_failed":
                ...
                logger.error(
                    "   Check FortiGate username/password and network connectivity"
                )
                return False
...
def test_session_caching():
    ...
    try:
        ...
        if active_sessions > 0:
            ...
            logger.warning(
                "   Session may not be persisting (check Redis configuration)"
            )
            return False
...
def test_api_endpoints():
    ...
    test_endpoints = ["cmdb/system/interface", "monitor/system/status"]
    ...
    if success_count == len(test_endpoints):
        ...
        logger.error(
            f"❌ Only {success_count}/{len(test_endpoints)} endpoints successful"
        )
        return False
...
def main():
    ...
    if failed_checks == 0:
        ...
        logger.info(
            "Your FortiGate dashboard is now using Redis session authentication."
        )
    else:
