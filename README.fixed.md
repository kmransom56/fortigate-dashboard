...
- `fixed_curl_command.sh`: A corrected curl command that uses the Authorization header.
- `fortigate_api_authentication_guide.md`: A comprehensive guide on FortiGate API authentication.
- `Dockerfile.fixed`: Updated Dockerfile for the dashboard service.
- `Dockerfile.wan_monitor.fixed`: Updated Dockerfile for the WAN monitor service.
- `docker-compose.fixed.yml`: Updated docker-compose file that uses the fixed Dockerfiles.
- `wan_monitor.fixed.py`: Updated WAN monitor script with the correct authentication method.
