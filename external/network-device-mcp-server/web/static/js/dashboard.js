...
async function loadOverviewData() {
    try {
        ...
        const brandsResponse = await apiCall('/brands');
        if (brandsResponse && brandsResponse.success && brandsResponse.brands) {
            const brandsData = { brands: brandsResponse.brands.map(brand => ({
                brand_code: brand.code,
                name: brand.name,
                description: `${brand.name} - ${brand.device_count} devices`
            }))};
            updateBrandsGrid(brandsData);
        }
        
        // Load FortiManager instances for stats
        const fmResponse = await apiCall('/fortimanager');
        if (fmResponse && fmResponse.success && fmResponse.data) {
...
function updateStatsGrid(data) {
    ...
    if (data && data.fortimanager_instances) {
        let totalDevices = data.total_estimated_devices || 0;
        let healthyDevices = 0;
        
        // Calculate healthy devices (assuming configured FortiManagers have healthy devices)
        data.fortimanager_instances.forEach(fm => {
            if (fm.status === 'configured') {
                healthyDevices += fm.estimated_devices || 0;
            }
        });
        
        document.getElementById('totalStores').textContent = totalDevices.toString();
        document.getElementById('healthyDevices').textContent = healthyDevices.toString();
...
async function loadBrandData(brandCode) {
    try {
        ...
        const response = await apiCall(`/brands/${brandCode}/overview`);
...
async function loadFortiManagerData() {
    try {
        ...
        const response = await apiCall('/fortimanager/');
...
async function viewFortiManagerDevices(fmName, adom = 'root') {
    try {
        ...
        const response = await apiCall(`/fortimanager/${fmName}/devices?adom=${adom}&limit=50`);
...
async function discoverAdoms(fmName) {
    try {
        ...
        const response = await apiCall(`/fortimanager/${fmName}/adoms`);
