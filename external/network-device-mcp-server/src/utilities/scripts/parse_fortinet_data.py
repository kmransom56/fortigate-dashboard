...
def find_port6_devices(devices: List[Dict]) -> List[Dict]:
    """
    Filter devices connected to port 6
    """
    port6_devices = []
    
    for device in devices:
        # Check for various port 6 formats
        port_num = device.get('port_number')
        raw_data = device.get('raw_data', '')
        
        # Direct port number match
        if port_num == 6:
            port6_devices.append(device)
        # Also check for text patterns in case regex missed some formats
        elif any(pat in raw_data.lower() for pat in ['port6','port 6','port06','port_6']):
            port6_devices.append(device)
    
    return port6_devices
...
def analyze_all_ports(devices: List[Dict]) -> Dict:
    ...
    if port_counts:
        ...
        if 6 in port_counts:
            print(f"\n*** PORT 6 FOUND: {port_counts[6]} devices ***")
        else:
            print("\n*** PORT 6 NOT FOUND in parsed data ***")
    
    return port_counts
    
def search_raw_text_for_port6(devices: List[Dict]) -> List[Dict]:
    """
    Search raw text for any mention of port 6, regardless of regex parsing
    """
    print(f"\nSEARCHING RAW TEXT for 'port6' patterns...")
    print("=" * 50)
    
    found_devices = []
    patterns_to_check = ['port6', 'port 6', 'port06', 'port_6']
    ...
    print(f"Found {len(found_devices)} devices with 'port6' patterns in raw text")
...
def create_report(port6_devices: List[Dict]) -> pd.DataFrame:
    """
    Create a pandas DataFrame report of port 6 devices
    """
    if not port6_devices:
        print("No devices found connected to port 6")
        return pd.DataFrame()
    
    df = pd.DataFrame(port6_devices)
...
def main():
    """
    Main function to process the Fortinet data and find port 6 devices
    """
    ...
    # Search raw text for port6 patterns
    raw_text_port6 = search_raw_text_for_port6(devices)
    
    print("\nSearching for devices on port 6...")
    port6_devices = find_port6_devices(devices)
    print(f"Found {len(port6_devices)} devices connected to port 6")
    
    if port6_devices:
        print("\nDevices connected to port 6:")
        print("=" * 50)
        
        # Create DataFrame report
        df_report = create_report(port6_devices)
        print(df_report.to_string(index=False))
        
        # Save to CSV for further analysis
        df_report.to_csv('port6_devices_report.csv', index=False)
        print(f"\nReport saved to 'port6_devices_report.csv'")
        
        # Print detailed information
        print("\nDetailed Information:")
        print("=" * 50)
        for i, device in enumerate(port6_devices, 1):
            print(f"\nDevice {i}:")
        ...
        if 'Port' in df.columns:
            ...
            for col in df.columns:
                if df[col].dtype == 'object':  # Text columns
                    port6_mask = df[col].str.contains('port6', case=False, na=False)
                    if port6_mask.any():
                        port6_devices = df[port6_mask]
                        break
