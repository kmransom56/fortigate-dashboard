...
try:
    ...
    print("✅ Enhanced Utilities system loaded")
except ImportError:
    print("Warning: Enhanced Utilities not available")
...
def discover_fortimanager_adoms(fm_name):
    ...
    try:
        # Test common ADOM names to find which ones have devices
        common_adoms = ['root', 'global', fm_name.lower(), f"{fm_name.lower()}_adom", 'main', 'production']
        adom_results = []
        ...
        for adom in common_adoms:
            try:
                ...
                if result.get('success') and result.get('data'):
                    device_count = result['data'].get('device_count', 0)
                    adom_results.append({
                        "adom": adom,
                        "device_count": device_count,
                        "status": "success",
                        "recommended": device_count > 100  # Flag high-count ADOMs
                    })
                else:
                    adom_results.append({
                        "adom": adom,
        ...
        return jsonify({
            ...
            "recommendation": "Use ADOM with highest device count for full device listing"
        })
    except Exception as e:
...
def fortimanager_devices(fm_name):
    ...
    if result.get('success') and result.get('data'):
        ...
        data['showing_devices'] = len(paginated_devices)
        data['offset'] = offset
        data['devices'] = paginated_devices
        
        if limit is not None:
            data['pagination'] = {
                'has_more': offset + len(paginated_devices) < total_devices,
                'next_offset': offset + len(paginated_devices) if offset + len(paginated_devices) < total_devices else None,
...
# =====================================================
# ENHANCED UTILITIES API ENDPOINTS (Phase 2A Integration)
# =====================================================

@app.route('/api/utilities/available', methods=['GET'])
def get_available_utilities():
    """GET /api/utilities/available - Get all available utilities with metadata"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    
    enhanced_utils = get_enhanced_utilities()
    result = enhanced_utils.get_available_utilities()
    return jsonify(result)

@app.route('/api/utilities/execute/<utility_name>', methods=['POST'])
def execute_utility(utility_name):
    """POST /api/utilities/execute/{utility_name} - Execute a specific utility with parameters"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    ...
    # Execute utility asynchronously
    import asyncio
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.execute_utility(utility_name, parameters))
    loop.close()
    
    return jsonify(result)

@app.route('/api/utilities/voice-command', methods=['POST'])
def execute_voice_command():
    """POST /api/utilities/voice-command - Execute utility via voice command"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    
    data = request.get_json() or {}
    voice_input = data.get('command', '')
    
    enhanced_utils = get_enhanced_utilities()
    
    # Execute voice command asynchronously
    import asyncio
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.execute_voice_command(voice_input))
    loop.close()
    
    return jsonify(result)

@app.route('/api/utilities/info/<utility_name>', methods=['GET'])
def get_utility_info(utility_name):
    """GET /api/utilities/info/{utility_name} - Get detailed info about a utility"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    
    enhanced_utils = get_enhanced_utilities()
    result = enhanced_utils.get_utility_info(utility_name)
    return jsonify(result)

@app.route('/api/utilities/voice-help', methods=['GET'])
def get_voice_help():
    """GET /api/utilities/voice-help - Get voice command help"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    
    enhanced_utils = get_enhanced_utilities()
    result = enhanced_utils.get_voice_help()
    return jsonify(result)
...
def run_device_discovery():
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.run_network_discovery(network, protocols))
    loop.close()
...
def run_snmp_discovery():
    """POST /api/utilities/snmp-discovery - Run SNMP device discovery"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    ...
    enhanced_utils = get_enhanced_utilities()
    parameters = {'brand': brand} if brand else {}
    if network:
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.execute_utility('unified_snmp_discovery', parameters))
    loop.close()
...
def run_ssl_diagnostics():
    ...
    host = data.get('host')
    port = data.get('port', 443)
    check_chain = data.get('check_chain', True)
    
    if not host:
        return jsonify({"success": False, "error": "Host parameter required"})
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.run_ssl_diagnostics(host, port, check_chain))
    loop.close()
...
def run_config_diff():
    ...
    if not device1 or not device2:
        return jsonify({"success": False, "error": "Both device1 and device2 parameters required"})
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.compare_fortigate_configs(device1, device2, sections))
    loop.close()
    
    return jsonify(result)

@app.route('/api/utilities/topology-visualizer', methods=['POST'])
def run_topology_visualizer():
    """POST /api/utilities/topology-visualizer - Generate network topology visualization"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    
    data = request.get_json() or {}
    input_data = data.get('input_data', 'auto-discover')
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.generate_topology_visualization(input_data, layout))
    loop.close()
...
@app.route('/api/utilities/ip-lookup', methods=['POST'])
def run_ip_lookup():
    """POST /api/utilities/ip-lookup - Lookup IP address information"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    ...
    if not ip_address:
        return jsonify({"success": False, "error": "IP address parameter required"})
    
    enhanced_utils = get_enhanced_utilities()
    parameters = {'ip_address': ip_address, 'detail_level': detail_level}
    
    import asyncio
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(enhanced_utils.execute_utility('ip_lookup', parameters))
    loop.close()
    
    return jsonify(result)

@app.route('/api/utilities/batch-operation', methods=['POST'])
def run_batch_operation():
    """POST /api/utilities/batch-operation - Run batch operations on multiple targets"""
    if not ENHANCED_UTILITIES_AVAILABLE:
        return jsonify({"success": False, "error": "Enhanced utilities not available"})
    ...
    if not operation or not targets:
        return jsonify({"success": False, "error": "Operation and targets parameters required"})
    
    enhanced_utils = get_enhanced_utilities()
    
    # Special handling for batch IP lookup
    if operation.lower() == 'ip lookup':
        import asyncio
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(enhanced_utils.batch_ip_lookup(targets))
        loop.close()
        return jsonify(result)
    
    # General batch operation handling
    results = []
    for target in targets:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        target_result = loop.run_until_complete(enhanced_utils.execute_utility(operation, parameters))
        loop.close()
