...
class BrandDetectionService:
    ...
    def detect_brand_from_ip(
        self, ip_address: str
    ) -> Tuple[Optional[RestaurantBrand], float]:
        """
    ...
    def detect_brand_from_inventory(
        self, ip_address: str
    ) -> Tuple[Optional[RestaurantBrand], float, Optional[str]]:
        """
        ...
        try:
            ...
            if location:
                ...
                )  # High confidence from inventory
    ...
    async def detect_infrastructure_type(
        self, ip_address: str, brand: RestaurantBrand
    ) -> InfrastructureType:
        """
    ...
    async def _probe_infrastructure_type(self, ip_address: str) -> InfrastructureType:
        ...
        try:
            ...
            try:
                ...
                if (
                    meraki_data
                    and not meraki_data.get("error")
                    and meraki_data.get("switches")
                ):
    ...
    async def discover_brand_devices(self, ip_address: str) -> BrandDetectionResult:
        ...
        detected_devices = await self._discover_devices_by_type(
        ...
        logger.info(
            f"Brand detection complete: {brand.value} ({confidence:.2f} confidence) - {len(detected_devices)} devices"
        )
        return result

    async def _discover_devices_by_type(
        self, infrastructure_type: InfrastructureType, ip_address: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _discover_fortigate_devices(
        self, ip_address: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _discover_fortiswitch_devices(
        self, ip_address: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _discover_meraki_devices(self, ip_address: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            switches_data = self.meraki_service.discover_restaurant_meraki_switches(
                org_filter
            )
    ...
    async def _discover_fortiap_devices(self, ip_address: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Simulated {len(devices)} FortiAP devices for {brand.value if brand else 'unknown'}"
            )
        except Exception as e:
    ...
    async def get_brand_topology_summary(
        self, brand_filter: str = None
    ) -> Dict[str, Any]:
        """
            ...
            for brand_name, brand_data in inventory_data.get("brands", {}).items():
                ...
                if brand_enum:
                    ...
                    summary["brands"][brand_name] = {
                        ...
                        "estimated_switches": brand_data.get(
                            "count", 0
                        ),  # 1 per location
                        "estimated_aps": brand_data.get("count", 0)
                        * (
                            4
                            if brand_enum == RestaurantBrand.SONIC
                            else 3 if brand_enum == RestaurantBrand.ARBYS else 6
                        ),
                        "infrastructure_type": expected_infra.get("topology_type"),
                    }

                    summary["total_devices"] += (
                        brand_data.get("count", 0) * 6
                    )  # Estimate 6 devices per location

            # Count infrastructure types
            for brand_info in summary["brands"].values():
                infra_type = brand_info.get("infrastructure_type", "unknown")
                summary["infrastructure_types"][infra_type] = summary[
                    "infrastructure_types"
                ].get(infra_type, 0) + brand_info.get("location_count", 0)
