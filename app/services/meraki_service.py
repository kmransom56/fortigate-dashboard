...
class MerakiService:
    ...
    def get_organizations(self) -> List[Dict[str, Any]]:
        ...
        for org in result:
            ...
            if any(
                brand in org_name
                for brand in ["bww", "buffalo wild wings", "arby", "arbys"]
            ):
    ...
    def get_network_devices(self, network_id: str) -> List[Dict[str, Any]]:
        ...
        switches = [
            device for device in result if device.get("model", "").startswith("MS")
        ]
        return switches
    ...
    def get_network_clients(
        self, network_id: str, timespan: int = 300
    ) -> List[Dict[str, Any]]:
    ...
    def discover_restaurant_meraki_switches(
        self, organization_filter: str = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Starting Meraki switch discovery with filter: {organization_filter}"
        )
        ...
        discovery_results = {
            ...
            "api_info": {"rate_limited": False, "requests_made": 0, "errors": []},
        }
        ...
        try:
            ...
            if isinstance(orgs, dict) and "error" in orgs:
                discovery_results["api_info"]["errors"].append(
                    f"Organizations: {orgs['error']}"
                )
            ...
            for org in filtered_orgs:
                ...
                if isinstance(networks, dict) and "error" in networks:
                    discovery_results["api_info"]["errors"].append(
                        f"Networks for {org_name}: {networks['error']}"
                    )
                    continue
                ...
                for network in networks:
                    ...
                    if isinstance(devices, dict) and "error" in devices:
                        discovery_results["api_info"]["errors"].append(
                            f"Devices for {network_name}: {devices['error']}"
                        )
                        continue
                    ...
                    for device in devices:
                        ...
                        switch_info = {
                            ...
                            "port_statuses": (
                                port_statuses
                                if not isinstance(port_statuses, dict)
                                or "error" not in port_statuses
                                else []
                            ),
                            "connected_clients": (
                                len(clients)
                                if not isinstance(clients, dict)
                                or "error" not in clients
                                else 0
                            ),
                            "device_type": "meraki_switch",
                            "management_ip": device.get(
                                "lanIp", device.get("wan1Ip", "Unknown")
                            ),
                        }
            ...
            logger.info(
                f"Meraki discovery complete: found {len(discovery_results['switches'])} switches across {len(filtered_orgs)} organizations"
            )
    ...
    def get_switch_topology_data(
        ...
        topology_data = {
            ...
            "enhancement_info": {
                ...
                "api_requests": discovery_data.get("api_info", {}).get(
                    "requests_made", 0
                ),
        ...
        for switch in discovery_data.get("switches", []):
            ...
            compatible_switch = {
                ...
                "status": (
                    "Authorized" if switch.get("status") == "online" else "Offline"
                ),
