...
class Icon3DService:
    ...
    def get_3d_icon_data(self, svg_path: str, device_type: str, device_name: str = "") -> Dict[str, Any]:
        """
    ...
    def _generate_3d_icon(self, svg_data_url: str, device_type: str, device_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("status") == "success":
                logger.info(f"Successfully generated 3D icon via Eraser AI for {device_type}")
                return self._process_eraser_result(result, device_type, device_name, svg_data_url)
            elif result.get("status") == "fallback":
                logger.info(f"Eraser AI fallback used for {device_type}")
                self.cache_data["stats"]["fallback_used"] += 1
                return self._get_fallback_3d_data(device_type, device_name, svg_data_url)
            else:
                logger.warning(f"Eraser AI generation failed for {device_type}: {result}")
                self.cache_data["stats"]["fallback_used"] += 1
                return self._get_fallback_3d_data(device_type, device_name, svg_data_url)
    ...
    def _process_eraser_result(self, result: Dict[str, Any], device_type: str, device_name: str, svg_data_url: str) -> Dict[str, Any]:
    ...
    def _get_fallback_3d_data(self, device_type: str, device_name: str, svg_data_url: str = None) -> Dict[str, Any]:
    ...
    async def batch_generate_3d_icons(self, svg_paths: List[Tuple[str, str, str]]) -> Dict[str, Any]:
        """
        ...
        async def generate_single_icon(svg_path: str, device_type: str, device_name: str):
            async with semaphore:
                try:
                    ...
                    return svg_path, self._get_fallback_3d_data(device_type, device_name)
