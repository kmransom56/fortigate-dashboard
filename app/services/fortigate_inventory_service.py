...
class FortiGateInventoryService:
    ...
    def _load_fortigate_inventory(self) -> None:
        ...
        try:
            with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
                    ...
                    try:
                        if '/' in ip_cidr:
                            ...
                            if '.' not in cidr:
                                ...
                                subnet_mask = str(ipaddress.IPv4Network(f"0.0.0.0/{cidr}", strict=False).netmask)
                            else:
                ...
                logger.info(f"Loaded {len(self.locations)} FortiGate locations from inventory")
                
                # Log brand distribution
                brand_counts = {}
                for location in self.locations.values():
                    brand_counts[location.brand] = brand_counts.get(location.brand, 0) + 1
    ...
    def get_locations_by_brand(self, brand: str, limit: int = None) -> List[FortiGateLocation]:
        """Get locations filtered by brand"""
        brand_locations = [
            loc for loc in self.locations.values() 
            if loc.brand.lower() == brand.lower()
        ]
    ...
    def get_all_locations(self, limit: int = None, offset: int = 0) -> List[FortiGateLocation]:
    ...
    def get_fortigate_connection_info(self, store_number: str) -> Optional[Dict[str, Any]]:
        ...
        connection_info = {
            ...
            "management_port": 8443,  # Standard HTTPS management port
            ...
            "management_url": f"https://{location.ip_address}:8443",
            "api_base_url": f"https://{location.ip_address}:8443/api/v2",
    ...
    def get_locations_for_discovery(self, brand: str = None, region: str = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        if region:
            locations = [loc for loc in locations if loc.region and loc.region.lower() == region.lower()]
        ...
        for location in locations:
            discovery_info = {
                ...
                "brand": location.brand,
                "region": location.region,
                "api_url": f"https://{location.ip_address}:8443/api/v2",
                "management_url": f"https://{location.ip_address}:8443",
    ...
    def search_locations(self, query: str, limit: int = 50) -> List[FortiGateLocation]:
        ...
        for location in self.locations.values():
            if (query in location.store_number.lower() or
                query in location.ip_address or
                query in location.brand.lower() or
                (location.region and query in location.region.lower())):
