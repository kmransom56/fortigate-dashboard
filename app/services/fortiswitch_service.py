...
import redis
import urllib3
from app.services.mac_vendors import (
    get_vendor_from_mac,
    get_vendor_icon,
    get_vendor_color,
)
...
FORTIGATE_SECRET = os.getenv("FORTIGATE_SECRET")
SESSION_KEY = "fortigate_session_active"
SESSION_TTL = int(os.getenv("FORTIGATE_SESSION_TTL", "600"))

# Cache key for switches
CACHE_KEY = "fortiswitches_cache"
CACHE_TTL = int(os.getenv("FORTISWITCH_CACHE_TTL", "60"))

# Persistent HTTP session (cookie + CSRF)
...
def login():
    if not FORTIGATE_USER or not FORTIGATE_SECRET:
        raise Exception("FortiGate login credentials missing")
    url = f"https://{FORTIGATE_HOST}/logincheck"
    resp = session.post(
        url,
        data={"username": FORTIGATE_USER, "secretkey": FORTIGATE_SECRET},
        timeout=20,
    )
...
def get_fortiswitches():
    ...
    # Try to return cached data to handle rate limits
    try:
        cached = redis_client.get(CACHE_KEY)
        if cached:
            logger.info("Returning FortiSwitch data from cache")
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            break
        if response is None:
            raise Exception("No response received from FortiGate API")
        ...
        if response.status_code == 429:
            logger.warning(
                "FortiGate API rate limit hit (429), returning cached or empty data"
            )
            try:
                cached = redis_client.get(CACHE_KEY)
                if cached:
            ...
            if response.status_code == 401:
                ...
                logger.error(
                    "Check your client IP and ensure it's within the allowed network"
                )
        ...
        processed = process_fortiswitch_data(data, dhcp_info)
        try:
            redis_client.set(CACHE_KEY, json.dumps(processed), ex=CACHE_TTL)
            logger.info(f"Cached FortiSwitch data for {CACHE_TTL} seconds")
        except Exception as e:
            logger.warning(f"Failed to cache FortiSwitch data: {e}")
        return processed
    except Exception as e:
        ...
        try:
            cached = redis_client.get(CACHE_KEY)
            if cached:
                logger.info("Returning cached FortiSwitch data after error")
                return json.loads(str(cached))
        except Exception:
            pass
        # Fallback: return empty list to avoid API errors
        return []
...
def change_fortiswitch_ip(switch_serial, new_ip, new_netmask="255.255.255.0"):
    ...
    try:
        ...
        # Ensure session and CSRF cookie are set
        ensure_logged_in()
        # Use session to send JSON PUT (session.verify honors SSL settings)
...
def get_dhcp_info():
    ...
    """
    # Ensure authenticated session and CSRF cookie
    ensure_logged_in()
    # Use persistent session for DHCP info
    url = f"https://{FORTIGATE_HOST}/api/v2/monitor/system/dhcp"

    mac_to_info = {}

    try:
        # SSL verification configurable via FORTIGATE_VERIFY_SSL env var
