...
class FortiGateMonitorService:
    ...
    def get_detected_devices(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in data:
                ...
                return {"devices": [], "total_devices": 0, "source": "monitor_api", "error": data.get("error")}
            ...
            logger.info(f"Retrieved {len(devices)} detected devices ({active_devices} active)")
            ...
            return {"devices": [], "total_devices": 0, "source": "monitor_api", "error": str(e)}
    ...
    def get_port_statistics(self, switch_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in data:
                ...
                return {"port_stats": [], "total_switches": 0, "source": "monitor_api", "error": data.get("error")}
                ...
                for switch_data in data["results"]:
                    ...
                    for port_name, stats in ports_data.items():
                        if isinstance(stats, dict):
                            ...
                            utilization_score = min(100, total_bytes / 1000000000)  # GB scale
                            ...
                            port_stat = {
                                ...
                                "health_status": "good" if stats.get("tx-errors", 0) == 0 and stats.get("rx-errors", 0) == 0 else "warning"
                            }
                    ...
                    switch_stat = {
                        ...
                        "avg_utilization": round(sum(p["utilization_score"] for p in port_stats) / len(port_stats), 2) if port_stats else 0
                    }
                    switch_stats.append(switch_stat)
            
            logger.info(f"Retrieved port statistics for {len(switch_stats)} switches, {total_ports} ports")
            ...
            return {"port_stats": [], "total_switches": 0, "source": "monitor_api", "error": str(e)}
    ...
    def get_comprehensive_device_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "timestamp": datetime.now().isoformat(),
                "statistics_coverage": len([d for d in enhanced_devices if d.get("has_statistics")]),
                "api_info": devices_data.get("api_info", {})
            }
            
        except Exception as e:
            logger.error(f"Exception getting comprehensive device data: {e}")
            return {"devices": [], "total_devices": 0, "source": "monitor_comprehensive", "error": str(e)}
    ...
    def get_port_devices(self, switch_id: str, port_name: str) -> List[Dict[str, Any]]:
        ...
        port_devices = []
        for device in devices_data.get("devices", []):
            if (device.get("switch_id") == switch_id and 
                device.get("port_name") == port_name):
