...
class HybridTopologyService:
    ...
    def _merge_topology_data(
        self, api_data: Dict[str, Any], snmp_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
            ...
            for switch in api_data.get("switches", []):
                ...
                for port in switch.get("ports", []):
                    ...
                    if port_name in snmp_device_by_port:
                        ...
                        logger.debug(
                            f"Enhanced {port_name} with SNMP device: {snmp_device.get('hostname')}"
                        )
                    else:
                ...
                total_connected = sum(
                    1 for port in enhanced_ports if port.get("connected_devices")
                )
    ...
    def get_enterprise_topology(self, org_filter: str = None) -> Dict[str, Any]:
        ...
        enterprise_data = {
            ...
            "discovery_time": None,
            "api_info": {"fortiswitch_requests": 0, "meraki_requests": 0, "errors": []},
        }
                ...
                try:
                    ...
                    if fortiswitch_data and not fortiswitch_data.get("error"):
                        ...
                        enterprise_data["api_info"]["errors"].append(
                            f"FortiSwitch: {fortiswitch_data.get('error', 'Unknown error')}"
                        )
                except Exception as e:
                    logger.error(f"FortiSwitch discovery failed: {e}")
                    enterprise_data["api_info"]["errors"].append(
                        f"FortiSwitch exception: {str(e)}"
                    )

            # Get Meraki data (for BWW and Arby's locations)
            if org_filter is None or org_filter in ["bww", "arbys"]:
                try:
                    meraki_data = self.meraki_service.get_switch_topology_data(
                        org_filter
                    )
                    if meraki_data and not meraki_data.get("error"):
                        ...
                        enterprise_data["api_info"]["errors"].append(
                            f"Meraki: {meraki_data.get('error', 'Unknown error')}"
                        )
                except Exception as e:
                    logger.error(f"Meraki discovery failed: {e}")
                    enterprise_data["api_info"]["errors"].append(
                        f"Meraki exception: {str(e)}"
                    )
