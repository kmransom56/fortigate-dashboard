...
def generate_3d_from_image(image_url: str) -> Dict:
    ...
    try:
        ...
        endpoints = ["/api/render/prompt", "/api/render/elements"]
            ...
            try:
                ...
                if endpoint == "/api/render/prompt":
                    ...
                    for i, payload in enumerate(payloads_to_try):
                        logger.info(
                            f"Trying prompt payload format {i + 1} for {endpoint}: {payload}"
                        )
                        res = requests.post(
                        ...
                        if res.status_code == 200:
                            ...
                            logger.warning(
                                f"Prompt payload format {i + 1} failed: {res.status_code} - {res.text[:200]}"
                            )
                    ...
                    for i, payload in enumerate(payloads_to_try):
                        logger.info(
                            f"Trying payload format {i + 1} for {endpoint}: {payload}"
                        )
                        res = requests.post(
                            url, json=payload, headers=headers, timeout=15
                        )
                        ...
                        if res.status_code == 200:
                            ...
                            logger.warning(
                                f"Payload format {i + 1} failed: {res.status_code} - {res.text[:200]}"
                            )
                else:
                ...
                if res.status_code == 200:
                    ...
                    logger.warning(
                        f"Eraser API error for {endpoint}: {res.status_code} - {error_msg}"
                    )
                    return {
                        "status": "error",
        ...
        return {
            ...
            "message": "All API endpoints failed, using fallback",
        }
