...
if CERT_PATH and CERT_PATH.startswith(
    "/home/keith/CascadeProjects/fortigate-dashboard/"
):
    # Extract the relative path and prepend with /app
    relative_path = CERT_PATH.replace(
        "/home/keith/CascadeProjects/fortigate-dashboard/", ""
    )
...
def get_interfaces():
    ...
    logger.info(
        "Using Authorization header with Bearer token instead of query parameter"
    )
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            logger.error(
                "Please check the token in the .env file or generate a new one"
            )

        # Check if the response is successful
        if response.status_code >= 400:
            logger.error(
                f"FortiGate API returned error status code: {response.status_code}"
            )
            logger.error(
            ...
            if response.status_code == 401:
                ...
                logger.error(
                    "2. IP restrictions: Only 192.168.0.0/24 network is allowed (trusthost)"
                )
                logger.error(
                    "3. CORS restrictions: Only https://192.168.0.20:80 is allowed"
                )
                logger.error(
...
def process_interface_data(data):
    ...
    # Format 3: Response is a dictionary with interface names as keys
        ...
        isinstance(data.get(key), dict) for key in data
    ):
