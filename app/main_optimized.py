...
async def warm_cache():
    ...
    try:
        ...
        interfaces, switches = await asyncio.gather(
            interfaces_task, switches_task, return_exceptions=True
        )
        ...
        logger.info(
            f"Cache pre-warmed with {len(interfaces) if isinstance(interfaces, dict) else 0} interfaces and {len(switches) if isinstance(switches, list) else 0} switches"
        )
...
async def read_home(request: Request):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "error.html", {"request": request, "error": "Failed to load home page"}
        )
...
async def show_dashboard(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        interfaces = await get_cached_data(
            "interfaces", get_interfaces_async, ttl=cache_ttl
        )
...
async def switches_page(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        switches = await get_cached_data(
            "switches", get_fortiswitches_optimized, ttl=cache_ttl
        )
        ...
        logger.info(
            f"Switches page loaded in {response_time:.2f}s ({total_switches} switches, {total_devices} devices)"
        )
        return response
        ...
        return templates.TemplateResponse(
            "error.html", {"request": request, "error": "Failed to load switches page"}
        )
