    ...
    try:
        ...
        if isinstance(switches_data, dict) and "success" in switches_data:
            if not switches_data.get("success", False):
                return {
                    "success": False,
                    "error": switches_data.get(
                        "message", "Failed to retrieve FortiSwitch data"
                    ),
                }
            ...
            processed_switches.append(
                {
                    ...
                    "model": switch.get(
                        "model",
                        (
                            switch.get("os_version", "").split("-")[0]
                            if switch.get("os_version")
                            else "Unknown"
                        ),
                    ),
                    "status": (
                        "online"
                        if switch.get("status", "").lower()
                        in ["connected", "authorized/up", "authorized"]
                        else "offline"
                    ),
                    "version": switch.get(
        ...
        logging.error(f"Error in /api/switches endpoint: {e}")
        return JSONResponse(
            status_code=200, content={"success": False, "error": str(e)}
        )
